...
// - "moonpacket" (case-insensitive) → render as "moonpacket" with brand styling
...
export const rehypeBrandword: Plugin = () => {
  ...
  return (tree: any) => {
    visit(tree, 'text', (node: any, index, parent: any) => {
      ...
      if (!value || !/(moonpacket|\$?moonini)/i.test(value)) return;
      const parts: string[] = value.split(/(moonpacket|\$?moonini)/i);
